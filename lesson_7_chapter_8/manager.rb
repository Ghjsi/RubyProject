...
class Manager
  ...
  def create_wagon
    ...
    puts 'Какой вагон нужо создать: 1 - грузовой, 2 - пассажирский. Вернуться в меню - 3.'
    ...
    when '1'
      ...
      CargoWagon.new(volume) # CargoWagon.all_instances
    when '2'
    ...
    puts 'Некорректный ввод. Характеристика вагона должна быть указана в форме неотрицательного целого числа.'
    create_wagon
  end
  ...
  def display_wagons
    ...
    puts 'Список поездов:'
    @trains.each_with_index { |train, index| puts "#{index} - #{train.num}" }
    puts 'Укажите номер поезда для которого нужно вывести список вагонов:'
    index = gets.chomp.to_i
  ...
  def take_place_or_fill
    ...
    train.composition.each_with_index { |wagon, index| puts "#{index} - #{wagon}"}
    puts 'Укажите номер вагона:'
    index = gets.chomp.to_i
    wagon = train.composition[index]
